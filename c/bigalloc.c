    ...
    */
	...
	if ( selection == 1 ) {
		...
		for (int i = 0; i < procNo; i++) { //rows
			printf("Enter maximum number of units process p%d will request from each resource (r0 to r%d): ", i, resCounter);
		...
		for (int i = 0; i < procNo; i++) { //rows
			printf("Enter number of units of each resource (r0 to r%d) allocatedTable to process p%d: ", resCounter, i);
			for (int j = 0; j < resNo; j++) { //cols
		...
		for (int i = 0; i < procNo; i++) { //rows
			for (int j = 0; j < resNo; j++) { //cols
				needTable[i * resNo + j] = maxClaimTable[i * resNo + j] - allocatedTable[i * resNo + j];
			}
		}
